...
namespace RStein::AsyncCpp::Detail
{
  ...
  void DataFlowBlockCommon<TInputItem, TOutputItem, TState>::Start()
  {
    ...
    for (auto& nextBlock : _outputNodes.MapSnapshot<typename DataFlow::IInputBlock<TOutputItem>::InputBlockPtr>([](auto &weakPtr){return weakPtr.lock();}))
    {
  ...
  {
    auto outputNodesSnapshot = _outputNodes.MapSnapshot<typename DataFlow::IInputBlock<TOutputItem>::InputBlockPtr>([](auto& weakPtr){return weakPtr.lock();});
